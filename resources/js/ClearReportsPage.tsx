...
function Document({data}: any) {
    ...
    return <article className={`report-document report-${data.standard.orientation || 'landscape'}`}>
        ...
        <section className="report-meta">
            ...
            <span>Scope: {data.report.department_id ? 'Selected department' : productionOnly ? 'Production flow' : 'Whole factory'}</span>
        </section>
        ...
        {data.standard.show_department_totals && <Table
            title="Department totals"
            headers={['Code', 'Department', 'Orders', 'Work entries', data.standard.input_label, data.standard.output_label, 'Damaged / rejected', 'Waste', 'Unit']}
            rows={(data.department_activity || []).map((row: any) => [
                row.code, row.name, row.production_orders, row.work_records,
                quantity(row.received_quantity, row.unit), quantity(row.completed_quantity, row.unit),
                quantity(row.damaged_quantity, row.unit), quantity(row.waste_quantity, row.unit), row.unit,
            ])}
        />}

        {!productionOnly && data.standard.show_stock_register && <Table
...
export default function ClearReportsPage({canExport, productionOnly = false}: any) {
    ...
    const [filters, setFilters] = useState({period: 'day', type: productionOnly ? 'production' : 'all', department_id: '', from: today, to: today});
