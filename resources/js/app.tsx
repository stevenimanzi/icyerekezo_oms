...
function UserSupportChat({user,locale}:{user:AuthUser;locale:Locale}) {
    const [tickets,setTickets]=useState<any[]>([]);const [activeId,setActiveId]=useState<number|null>(null);const [creating,setCreating]=useState(false);const [busy,setBusy]=useState(false);const [loading,setLoading]=useState(true);const [error,setError]=useState('');const [message,setMessage]=useState('');const [lastUpdated,setLastUpdated]=useState<Date|null>(null);const messageEnd=useRef<HTMLDivElement|null>(null);const [form,setForm]=useState({subject:'',message:'',category:'general',priority:'normal'});
    ...
    useEffect(()=>{if(tickets.length&&!tickets.some(item=>item.id===activeId))setActiveId(tickets[0].id);if(!tickets.length)setActiveId(null)},[tickets,activeId]);
    const active=tickets.find(item=>item.id===activeId);
    useEffect(()=>{messageEnd.current?.scrollIntoView?.({behavior:'smooth',block:'end'})},[active?.messages?.length,activeId]);
    const create=async(e:React.FormEvent)=>{e.preventDefault();setBusy(true);setError('');try{const ticket:any=await api('/api/support/tickets',{method:'POST',body:JSON.stringify(form)});setCreating(false);setForm({subject:'',message:'',category:'general',priority:'normal'});await load(true);setActiveId(Number(ticket.id))}catch(reason){setError(reason instanceof Error?reason.message:'Unable to send your question.')}finally{setBusy(false)}};
    const send=async()=>{const text=message.trim();if(!text||!active||busy)return;setBusy(true);setError('');try{await api(`/api/support/tickets/${active.id}/reply`,{method:'POST',body:JSON.stringify({message:text})});setMessage('');await load(true)}catch(reason){setError(reason instanceof Error?reason.message:'Unable to send your message.')}finally{setBusy(false)}};
    return <section className="module-page"><div className="module-hero"><div className="module-title"><span><MessageSquare size={22}/></span><div><div className="eyebrow"><i></i>{locale==='en'?'SUPPORT ONLINE':'SUPPORT EN LIGNE'}</div><h1>{locale==='en'?'Help and support':'Aide et support'}</h1><p>{locale==='en'?'Chat securely with the system administrator and track every request.':"Contactez l'administrateur et suivez chaque demande."}</p></div></div><div className="support-actions"><span className="support-live"><i></i>{lastUpdated?'Updated '+lastUpdated.toLocaleTimeString():'Connecting...'}</span><button className="secondary-btn" disabled={loading} onClick={()=>load()}><RefreshCw size={16}/>Refresh</button><button className="primary-btn" onClick={()=>setCreating(!creating)}><MessageSquare size={17}/>{creating?'Close form':(locale==='en'?'New question':'Nouvelle question')}</button></div></div>{error&&<div className="admin-alert error">{error}</div>}{creating&&<form className="admin-form panel support-new" onSubmit={create}><h2>{locale==='en'?'Ask a new question':'Poser une nouvelle question'}</h2><div className="form-grid"><label>{locale==='en'?'Subject':'Sujet'}<input required maxLength={180} value={form.subject} onChange={e=>setForm({...form,subject:e.target.value})}/></label><label>Category<select value={form.category} onChange={e=>setForm({...form,category:e.target.value})}><option value="general">General</option><option value="technical">Technical</option><option value="billing">Billing</option><option value="security">Security</option><option value="data">Data</option></select></label><label>Priority<select value={form.priority} onChange={e=>setForm({...form,priority:e.target.value})}><option value="low">Low</option><option value="normal">Normal</option><option value="high">High</option><option value="urgent">Urgent</option></select></label></div><label>{locale==='en'?'Your question':'Votre question'}<textarea required maxLength={10000} value={form.message} onChange={e=>setForm({...form,message:e.target.value})}/></label><button className="primary-btn" disabled={busy}>{busy?'Sending...':'Send question'}</button></form>}<div className="support-room panel"><aside className="chat-list">{loading&&!tickets.length?<div className="chat-empty">Loading conversations...</div>:tickets.length?tickets.map(item=>{const messages=item.messages as any[];const latest=messages.length?messages[messages.length-1]:null;return <button key={item.id} className={item.id===activeId?'active':''} onClick={()=>setActiveId(item.id)}><span>{item.subject.slice(0,2).toUpperCase()}</span><div><b>{item.subject}</b><small>{String(latest?.message||'No messages')}</small><em>{item.status.replaceAll('_',' ')} · {item.priority}</em></div></button>}):<div className="chat-empty">No questions yet. Start a conversation.</div>}</aside>{active?<section className="chat-window"><header><div><h3>{active.subject}</h3><p>{active.ticket_number} · {active.category}</p></div><div className="chat-ticket-status"><span className={`admin-status ${active.status}`}>{active.status.replaceAll('_',' ')}</span><span className={`admin-status ${active.priority}`}>{active.priority}</span></div></header><div className="chat-messages">{active.messages.length?active.messages.map((item:any)=><div key={item.id} className={Number(item.user_id)===user.id?'chat-message admin':'chat-message user'}><b>{Number(item.user_id)===user.id?(locale==='en'?'You':'Vous'):(item.user?.name||'System administrator')}</b><p>{String(item.message||'')}</p><time>{item.created_at?new Date(item.created_at).toLocaleString():''}</time></div>):<div className="chat-empty">No messages in this conversation.</div>}<div ref={messageEnd}/></div><div className="chat-compose"><textarea aria-label="Message" maxLength={10000} placeholder={locale==='en'?'Write a message...':'Ecrire un message...'} value={message} onChange={e=>setMessage(e.target.value)} onKeyDown={e=>{if(e.key==='Enter'&&!e.shiftKey){e.preventDefault();send()}}}/><button className="primary-btn" disabled={busy||!message.trim()} onClick={send}>{busy?'Sending...':(locale==='en'?'Send':'Envoyer')}</button></div></section>:<section className="chat-window chat-welcome"><MessageSquare size={36}/><h3>{locale==='en'?'Select a conversation':'Choisissez une conversation'}</h3><p>{locale==='en'?'Choose a request or create a new question.':'Choisissez une demande ou creez une question.'}</p></section>}</div></section>
}
