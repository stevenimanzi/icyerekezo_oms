...
function UserSupportChat({user,locale}:{user:AuthUser;locale:Locale}) {
    ...
    return <section className="module-page support-page"><div className="module-hero"><div className="module-title"><span><MessageSquare size={22}/></span><div><div className="eyebrow"><i></i>{locale==='en'?'SUPPORT ONLINE':'SUPPORT EN LIGNE'}</div><h1>{locale==='en'?'Chat with support':'Discuter avec le support'}</h1><p>{locale==='en'?'Send a message directly to the system administrator.':"Envoyez directement un message à l'administrateur système."}</p></div></div><div className="support-actions"><span className="support-live"><i></i>{lastUpdated?(locale==='en'?'Online · updated ':'En ligne · actualisé ')+lastUpdated.toLocaleTimeString():(locale==='en'?'Connecting...':'Connexion...')}</span><button className="secondary-btn" disabled={loading} onClick={()=>load()}><RefreshCw size={16}/>{locale==='en'?'Refresh':'Actualiser'}</button></div></div>{error&&<div className="admin-alert error">{error}</div>}<div className="support-room user-support-room panel"><section className="chat-window"><header><div className="support-admin-avatar"><MessageSquare size={20}/></div><div className="support-admin-name"><h3>{locale==='en'?'ICYEREKEZO Support':'Support ICYEREKEZO'}</h3><p><i></i>{locale==='en'?'System administrator online':'Administrateur système en ligne'}</p></div></header><div className="chat-messages">{loading&&!active?<div className="chat-empty">{locale==='en'?'Loading messages...':'Chargement des messages...'}</div>:active?.messages?.length?active.messages.map((item:any)=><div key={item.id} className={Number(item.user_id)===user.id?'chat-message admin':'chat-message user'}><b>{Number(item.user_id)===user.id?(locale==='en'?'You':'Vous'):(item.user?.name||'System administrator')}</b><p>{String(item.message||'')}</p><time>{item.created_at?new Date(item.created_at).toLocaleString():''}</time></div>):<div className="chat-welcome-simple"><MessageSquare size={34}/><h3>{locale==='en'?'Welcome to support':'Bienvenue au support'}</h3><p>{locale==='en'?'Type your message below. Our system administrator will reply here.':'Écrivez votre message ci-dessous. Notre administrateur vous répondra ici.'}</p></div>}<div ref={messageEnd}/></div><div className="chat-compose"><textarea aria-label={locale==='en'?'Message':'Message'} maxLength={10000} rows={1} placeholder={locale==='en'?'Type a message...':'Écrire un message...'} value={message} onChange={e=>setMessage(e.target.value)} onKeyDown={e=>{if(e.key==='Enter'&&!e.shiftKey){e.preventDefault();send()}}}/><button className="primary-btn" disabled={busy||!message.trim()} onClick={send}>{busy?(locale==='en'?'Sending...':'Envoi...'):(locale==='en'?'Send':'Envoyer')}</button></div></section></div></section>
}
