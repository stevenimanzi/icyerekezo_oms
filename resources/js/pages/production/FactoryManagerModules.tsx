...
function Alert({ error, success }: { error: string; success: string }) {
...
export function FlowSetupPage() {
    ...
    const apply = async () => {
        ...
        setSuccess('');
        try {
            const result = await api('/api/factory/flow-suggestion/apply', { method: 'POST' });
            setSuccess(result.message || 'Recommended departments, workstations and workflow stages are ready.');
            await load();
        } catch (reason: any) {
    ...
    return (
        <section className="module-page">
            <Heading
                title="Factory departments and work steps" description="Create the departments and ordered work steps used by your factory."
                action={<button className="primary-btn" disabled={busy || !suggestion} onClick={apply}><Save size={17} />{busy ? 'Applying…' : 'Use recommended setup'}</button>}
            />
            <Alert error={error} success={success} />
            <article className="panel flow-suggestion">
                <div className="flow-head"><div><small>FACTORY TYPE</small><h2>{String(data?.industry || '').replaceAll('_', ' ')}</h2><p>{suggestion?.name}</p></div></div>
                <div className="flow-line">
                    {suggestion?.departments?.map((item: any, index: number) => <div key={item.code}><span>{index + 1}</span><b>{item.name}</b><small>{item.workstation_type}</small></div>)}
                </div>
            </article>
        </section>
    );
...
export function ProductionFlowPage() {
    ...
    const save = async (e: React.FormEvent) => {
        ...
        try {
            ...
            setEditing(null);
            setSuccess('Factory working flow saved successfully.');
            await load();
        } catch (reason: any) {
            setError(reason.message);
        } finally { setBusy(false); }
    ...
    const applyRecommended = async () => {
        ...
        try {
            const result = await api('/api/factory/flow-suggestion/apply', { method: 'POST' });
            setSuccess(result.message || 'Recommended industry flow applied. You can now customize every stage.');
            await load();
        } catch (reason: any) {
            setError(reason.message);
        } finally { setBusy(false); }
    ...
    return (
        <section className="module-page">
            <Heading
                title="Production steps" description="Set the order of work, required employees and quality checks used by your factory."
                action={(
                    <div className="workflow-actions production-step-actions">
                        <button className="secondary-btn" onClick={load}><RefreshCw size={16} />Refresh</button>
                        <button className="secondary-btn" disabled={busy} onClick={applyRecommended}><Factory size={16} />Use recommended steps</button>
                        <button className="primary-btn" onClick={create}><Plus size={16} />Create work steps</button>
                    </div>
                )}
            />
            <Alert error={error} success={success} />
            ...
            {editing && (
                <form className="panel admin-form workflow-editor" onSubmit={save}>
                    <header>
                        <div><small>FACTORY FLOW CONFIGURATION</small><h2>{editing.id ? 'Edit working flow' : 'Create working flow'}</h2></div>
                        <button type="button" className="icon-button" onClick={() => setEditing(null)}><X size={18} /></button>
                    </header>
                    <div className="form-grid">
                        <label>Workflow name<input required value={editing.name} onChange={e => setEditing({ ...editing, name: e.target.value })} /></label>
                        <label>Workflow code<input required maxLength={40} value={editing.code} onChange={e => setEditing({ ...editing, code: e.target.value.toUpperCase() })} /></label>
                        <label>Description<textarea value={editing.description || ''} onChange={e => setEditing({ ...editing, description: e.target.value })} /></label>
                        <label>Status<select value={editing.status} onChange={e => setEditing({ ...editing, status: e.target.value })}><option value="active">Active</option><option value="inactive">Inactive</option></select></label>
                    </div>
                    <div className="stage-editor-head">
                        <div><h3>Working stages</h3><p>Stages run from top to bottom. Stage codes are created automatically from their names.</p></div>
                        <button type="button" className="secondary-btn" onClick={() => setEditing({ ...editing, stages: [...editing.stages, blankStage()] })}><Plus size={15} />Add stage</button>
                    </div>
                    <div className="stage-option-help">
                        <div><b>Quality check</b><span>Enable this when Quality Control must inspect the output and record whether it passes or has defects.</span></div>
                        <div><b>Approval</b><span>Enable this when the department manager must review and confirm the stage result.</span></div>
                    </div>
                    <div className="stage-editor-list">
                        {editing.stages.map((item: any, index: number) => (
                            <article key={item.id || index}>
                                ...
                                <div className="stage-order">
                                    <button type="button" disabled={index === 0} onClick={() => move(index, -1)} title="Move up"><ArrowUp size={16} /></button>
                                    <button type="button" disabled={index === editing.stages.length - 1} onClick={() => move(index, 1)} title="Move down"><ArrowDown size={16} /></button>
                                    <button type="button" disabled={editing.stages.length === 1} onClick={() => remove(index)} title="Remove stage"><Trash2 size={16} /></button>
                                </div>
                            </article>
                        ))}
                    </div>
            ...
            <div className="workflow-list">
                {(data?.workflows || []).length ? data.workflows.map((workflow: any) => (
                    <article className="panel" key={workflow.id}>
                        <header>
                            <div><small>{workflow.code}</small><h2>{workflow.name}</h2><p>{workflow.description}</p></div>
                            <div className="workflow-card-actions"><span className={'admin-status ' + workflow.status}>{String(workflow.status).charAt(0).toUpperCase() + String(workflow.status).slice(1)}</span><button className="table-action" onClick={() => edit(workflow)}><Edit3 size={15} />Configure</button></div>
                        </header>
                        <div className="flow-line">
                            {workflow.stages.map((stage: any) => (
                                <div key={stage.id}><span>{stage.sequence}</span><b>{stage.name}</b><small>{stage.required_workers || 1} worker(s){stage.quality_required ? ' · Quality check' : ''}{stage.approval_required ? ' · Approval required' : ''}</small></div>
                            ))}
                        </div>
                    </article>
                )) : (
                    <div className="panel empty-state">
                        ...
                        <div className="empty-actions">
                            <button className="secondary-btn" onClick={applyRecommended}>Use suggested flow</button>
                            <button className="primary-btn" onClick={create}>Create workflow</button>
                        </div>
                    </div>
                )}
            </div>
...
export function TeamManagementPage() {
    ...
    const create = async (e: React.FormEvent) => {
        ...
        try {
            ...
            setShowCreate(false);
            setSuccess('Employee account created and assigned successfully.');
            await load();
        } catch (reason: any) {
            setError(reason.message);
    ...
    const save = async (user: any) => {
        ...
        setSuccess('');
        try {
            await api('/api/team/users/' + user.id, { method: 'PATCH', body: JSON.stringify(values) });
            setSuccess('Employee access and assignment updated.');
            setEditing(current => { const next = { ...current }; delete next[user.id]; return next; });
            await load();
        } catch (reason: any) {
            setError(reason.message);
        }
    ...
    return (
        <section className="module-page team-management-page">
            <Heading
                title="Employees" description="Manage factory employees and school accounts associated with this factory."
                action={activeTab === 'employees' ? <button className="primary-btn" onClick={() => { setError(''); setShowCreate(true); }}><Plus size={17} />Add employee</button> : null}
            />
            <Alert error={error} success={success} />
            ...
            {activeTab === 'employees' && (
                <>
                    <section className="team-user-metrics">
                        {metrics.map(item => <article className="panel" key={item.label}><span className={item.tone || ''}>{item.icon}</span><div><small>{item.label}</small><strong>{item.value}</strong><p>{item.detail}</p></div></article>)}
                    </section>
                    <div className="admin-table-wrap team-users-table">
                        <table className="admin-table">
                            <thead><tr><th>Employee</th><th>Status</th><th>Role</th><th>Department</th><th>Action</th></tr></thead>
                            <tbody>
                                {users.length ? users.map((user: any) => {
                                    ...
                                    return (
                                        <tr key={user.id}>
                                            ...
                                            <td>
                                                <select value={active ? '1' : '0'} onChange={e => setEditing({ ...editing, [user.id]: { ...current, is_active: e.target.value === '1' } })}>
                                                    <option value="1">Active</option><option value="0">Inactive</option>
                                                </select>
                                            </td>
                                            <td>
                                                <select value={current.role_id || user.roles?.[0]?.id || ''} onChange={e => setEditing({ ...editing, [user.id]: { ...current, role_id: e.target.value } })}>
                                                    <option value="">Choose role</option>
                                                    {(data?.roles || []).map((item: any) => <option key={item.id} value={item.id}>{item.name}</option>)}
                                                </select>
                                            </td>
                                            <td>
                                                <select value={selectedDepartment} onChange={e => setEditing({ ...editing, [user.id]: { ...current, department_id: e.target.value || null } })}>
                                ...
                                }) : <tr><td className="empty-cell" colSpan={5}>No employees have been created yet.</td></tr>}
                            </tbody>
                        </table>
                    </div>
                </>
            ...
            {activeTab === 'schools' && (
                <div className="admin-table-wrap team-users-table">
                    ...
                    <table className="admin-table">
                        <thead><tr><th>School Account</th><th>School Details</th><th>Status</th><th>Role</th><th>Action</th></tr></thead>
                        <tbody>
                            {schoolUsers.length ? schoolUsers.map((user: any) => {
            ...
            {showCreate && (
                <div className="team-modal-backdrop" role="presentation" onMouseDown={event => { if (event.target === event.currentTarget) setShowCreate(false); }}>
                    <section className="team-modal" role="dialog" aria-modal="true" aria-labelledby="create-user-title">
                        <header>
                            <div><div><h2 id="create-user-title">Create employee account</h2><p>Add login access and assign the employee to the correct team.</p></div></div>
                            <button type="button" aria-label="Close" onClick={() => setShowCreate(false)}><X size={19} /></button>
                        </header>
                        <form className="admin-form employee-create-form" onSubmit={create}>
                            <div className="form-grid employee-form-grid">
                                <label>Full name<input autoFocus required placeholder="Enter employee name" value={newUser.name} onChange={e => setNewUser({ ...newUser, name: e.target.value })} /></label>
                                <label>Email address<input required type="email" placeholder="name@company.com" value={newUser.email} onChange={e => setNewUser({ ...newUser, email: e.target.value })} /></label>
                                <label>
                                    Role
                                    <select required value={newUser.role_id} onChange={e => setNewUser({ ...newUser, role_id: e.target.value })}>
                                        <option value="">Choose role</option>
                                        {(data?.roles || []).map((item: any) => <option key={item.id} value={item.id}>{item.name}</option>)}
                                    </select>
                                </label>
                                ...
                                <label>Temporary password<input required minLength={10} type="password" placeholder="Create a secure password" value={newUser.password} onChange={e => setNewUser({ ...newUser, password: e.target.value })} /><small>Use uppercase, lowercase, number and symbol.</small></label>
                                <label>Confirm password<input required minLength={10} type="password" placeholder="Enter the password again" value={newUser.password_confirmation} onChange={e => setNewUser({ ...newUser, password_confirmation: e.target.value })} /></label>
                            </div>
                            <footer>
            ...
            {resetUser && (
                <div className="team-modal-backdrop" role="presentation" onMouseDown={event => { if (event.target === event.currentTarget) setResetUser(null); }}>
                    <section className="team-modal" role="dialog" aria-modal="true" aria-labelledby="reset-password-title">
                        <header>
                            <div><div><h2 id="reset-password-title">Reset User Password</h2><p>Set a new password for {resetUser.name}</p></div></div>
                            <button type="button" aria-label="Close" onClick={() => setResetUser(null)}><X size={19} /></button>
                        </header>
                        <form className="admin-form employee-create-form" onSubmit={handleResetPassword}>
                            ...
                            <footer>
                                <button type="button" className="secondary-btn" onClick={() => setResetUser(null)}>Cancel</button>
                                <button className="primary-btn" disabled={resetting}>{resetting ? 'Resetting...' : 'Reset Password'}</button>
                            </footer>
                        </form>
                    </section>
                </div>
...
export function ReportsPage({ canExport, productionOnly = false }: { canExport: boolean; productionOnly?: boolean }) {
    ...
    const load = async (silent = false) => {
        ...
        try {
            ...
            setData(await api('/api/reports?' + query.toString()));
    ...
    useEffect(() => {
        ...
        const refresh = async (silent = false) => {
            ...
            try {
                const query = new URLSearchParams(filters);
                const result = await api('/api/reports?' + query.toString());
                if (active) {
                    setData(result);
                    setError('');
                    setLastUpdated(new Date());
        ...
        const debounce = window.setTimeout(() => refresh(false), 250);
        const timer = window.setInterval(() => refresh(true), 5000);
        return () => { active = false; window.clearTimeout(debounce); window.clearInterval(timer); };
    }, [filters.period, filters.type, filters.department_id, filters.from, filters.to]);
    ...
    return (
        <section className="module-page report-page">
            <div className="no-print">
                <Heading
                    icon={FileText}
                    title={productionOnly ? 'Production flow reports' : 'Factory activity reports'}
                    description={productionOnly ? 'Review real output, rejection, waste, downtime and progress for every category in the configured production flow.' : 'Review operational results for every department, including departments with no activity.'}
                    action={canExport ? <button className="primary-btn" disabled={!data} onClick={() => window.print()}><Printer size={17} />Print / Save PDF</button> : null}
                />
                ...
                <div className="panel report-filters">
                    <label>
                        ...
                        <select value={filters.period} onChange={e => setFilters({ ...filters, period: e.target.value })}>
                            <option value="day">Today</option><option value="week">Last 7 days</option><option value="month">This month</option><option value="custom">Custom dates</option>
                        </select>
                    </label>
                    {!productionOnly && (
                        <label>
                            ...
                            <select value={filters.type} onChange={e => setFilters({ ...filters, type: e.target.value })}>
                                <option value="all">Complete factory activity</option><option value="departments">Department performance</option>
                                <option value="production">Production stages</option><option value="inventory">Inventory movements</option><option value="activity">Operational events</option>
                            </select>
                        </label>
                    )}
                    <label>
                        {productionOnly ? 'Production flow category' : 'Department'}
                        <select value={filters.department_id} onChange={e => setFilters({ ...filters, department_id: e.target.value })}>
                            <option value="">{productionOnly ? 'All production flow categories' : 'All departments'}</option>
                            {(data?.filters?.departments || []).map((item: any) => <option key={item.id} value={item.id}>{item.name}</option>)}
                        </select>
                    </label>
                    ...
                    <button className="secondary-btn" disabled={loading} onClick={() => load(false)}><RefreshCw size={16} />{loading ? 'Updating...' : 'Refresh now'}</button>
                    <div className="live-report-status"><i></i><span>Live data{lastUpdated ? ' · Updated ' + lastUpdated.toLocaleTimeString() : ''}</span></div>
                </div>
            </div>
...
function ReportDocument({ data }: any) {
    ...
    return (
        <article className="report-document">
            <header>
                <div><h1>{data.factory.name}</h1><p>{String(data.factory.industry_type).replaceAll('_', ' ')}</p></div>
                <div><b>{productionOnly ? 'PRODUCTION FLOW REPORT' : 'FACTORY ACTIVITY REPORT'}</b><span>{data.report.from} to {data.report.to}</span></div>
            </header>
            <section className="report-meta">
                ...
                <span>Scope: {data.report.department_id ? 'Selected flow category' : productionOnly ? 'All production flow categories' : 'All departments'}</span>
            </section>
            <div className="report-summary">
                {Object.entries(summary).map(([key, value]: any) => <div key={key}><span>{key.replaceAll('_', ' ')}</span><b>{Number(value).toLocaleString()}</b></div>)}
            </div>
            {productionOnly ? (
                <ReportTable
                    title="Production flow category performance"
                    headers={['Code', 'Flow category', 'Orders', 'Stages', 'Completed', 'In progress', 'Output', 'Rejected', 'Waste', 'Downtime (min)']}
                    rows={(data.department_activity || []).map((item: any) => [item.code, item.name, item.production_orders, item.stages_processed, item.completed_stages, item.in_progress_stages, item.output_quantity, item.rejected_quantity, item.waste_quantity, item.downtime_minutes])}
                />
            ) : (
                <ReportTable
                    title="Department activity and performance"
                    headers={['Code', 'Department', 'Manager', 'Employees', 'Orders', 'Stages', 'Completed', 'In progress', 'Output', 'Rejected', 'Waste', 'Downtime (min)', 'Events']}
                    rows={(data.department_activity || []).map((item: any) => [item.code, item.name, item.manager || 'Not assigned', item.employees, item.production_orders, item.stages_processed, item.completed_stages, item.in_progress_stages, item.output_quantity, item.rejected_quantity, item.waste_quantity, item.downtime_minutes, item.operational_events])}
                />
            )}
            {data.production.length > 0 && (
                <ReportTable
                    title="Production stage activity"
                    headers={['Updated', 'Order', 'Product', 'Stage', 'Status', 'Output', 'Rejected', 'Waste', 'Downtime']}
                    rows={data.production.map((item: any) => [new Date(item.updated_at).toLocaleString(), item.order_number, item.product_name || '—', item.stage_name, item.status, item.output_quantity, item.rejected_quantity, item.waste_quantity, item.downtime_minutes])}
                />
            )}
            {!productionOnly && data.inventory.length > 0 && (
                <ReportTable
                    title="Inventory movement activity"
                    headers={['Date', 'Item', 'Code', 'Warehouse', 'Type', 'Quantity', 'Cost', 'Balance', 'Reason']}
                    rows={data.inventory.map((item: any) => [new Date(item.occurred_at).toLocaleString(), item.item_name, item.sku, item.warehouse_name, item.type, item.quantity_delta, item.unit_cost, item.balance_after, item.reason || '—'])}
                />
            )}
            {!productionOnly && data.activities.length > 0 && (
                <ReportTable
                    title="Operational events"
                    headers={['Date', 'Recorded by', 'Event', 'Description']}
                    rows={data.activities.map((item: any) => [new Date(item.created_at).toLocaleString(), item.user?.name || 'System', item.event.replaceAll('.', ' / '), item.description])}
                />
            )}
...
function ReportTable({ title, headers, rows }: any) {
    return (
        <section className="report-section">
            <h2>{title}</h2>
            <table>
                <thead><tr>{headers.map((item: string) => <th key={item}>{item}</th>)}</tr></thead>
                <tbody>{rows.map((row: any[], index: number) => <tr key={index}>{row.map((cell: any, cellIndex: number) => <td key={cellIndex}>{cell}</td>)}</tr>)}</tbody>
            </table>
        </section>
    );
}
