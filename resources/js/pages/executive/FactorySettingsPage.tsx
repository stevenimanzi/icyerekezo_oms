...
const Check = ({ label, note, checked, change }: any) => (
    <label className="switch-line"><span><b>{label}</b><small>{note}</small></span><input type="checkbox" checked={!!checked} onChange={event => change(event.target.checked)} /></label>
);
...
export default function FactorySettingsPage() {
    ...
    if (!form) {
        return (
            <section className="module-page">
                <div className="module-hero"><div className="module-title"><span><Settings /></span><div><h1>Factory settings</h1><p>Manage your factory profile and operating rules.</p></div></div></div>
                {error && <div className="admin-alert error">{error}</div>}
                <div className="panel admin-loading">Loading factory settings…</div>
            </section>
        );
    }
    ...
    const reportDepartments = data?.report_departments || [];
    const toggleReportDepartment = (id: number) => {
        const selected = reportSettings.department_ids || [];
        if (selected.length === 1 && selected.includes(id)) return;
        report('department_ids', selected.includes(id) ? selected.filter((item: number) => item !== id) : [...selected, id]);
    };

    return (
        <section className="module-page factory-settings-page">
            ...
            <nav className="panel factory-settings-tabs" aria-label="Factory settings sections">
                {tabs.map(({ key, label, icon: Icon }) => (
                    <button type="button" key={key} className={activeTab === key ? 'active' : ''} onClick={() => { setActiveTab(key); setSuccess(''); }}>
            ...
            <form id="factory-settings-form" onSubmit={save} className="factory-settings-grid">
                <div className="factory-settings-main">
                    {activeTab === 'profile' && (
                        <Card icon={Building2} title="Factory profile" note="Official information shown on reports and operational documents.">
                    ...
                    {activeTab === 'location' && (
                        <Card icon={MapPin} title="Location and regional settings" note="Controls report dates, language and money values.">
                            <div className="form-grid">
                    ...
                    {activeTab === 'schedule' && (
                        <Card icon={Clock3} title="Working schedule" note="Define normal operating hours and working days.">
                            <div className="form-grid">
                            ...
                            <div className="working-days">
                                {days.map(day => <button type="button" key={day} className={form.settings.working_days.includes(day) ? 'active' : ''} onClick={() => toggleDay(day)}>{day.slice(0, 3)}</button>)}
                            </div>
                        </Card>
                    )}

                    {activeTab === 'controls' && (
                        <Card icon={ShieldCheck} title="Operating controls" note="Safety rules for production and stock recording.">
                            <div className="form-grid">
                    ...
                    {activeTab === 'reports' && (
                        <Card icon={FileText} title="Report design" note="Choose the daily report your factory needs. Changes apply to live reports and printed PDF documents.">
                            <div className="form-grid">
                                ...
                                <Field label="Report footer"><input placeholder="Generated by ICYEREKEZO OMS" value={reportSettings.footer_text || ''} onChange={event => report('footer_text', event.target.value)} /></Field>
                                <Field label="Default report period">
                                    <select value={reportSettings.default_period} onChange={event => report('default_period', event.target.value)}>
                                        <option value="day">Today</option><option value="week">Last 7 days</option><option value="month">This month</option>
                                    </select>
                                </Field>
                                <Field label="PDF page layout">
                                    <select value={reportSettings.orientation} onChange={event => report('orientation', event.target.value)}>
                                        <option value="landscape">Landscape — wide tables</option><option value="portrait">Portrait — normal page</option>
                                    </select>
                                </Field>
                                ...
                                <Field label="Product details to record" note="Separate items with commas. Example: Product, Batch, Colour, Size">
                                    <input placeholder="Product, Batch, Type, Grade" value={(reportSettings.attributes || []).join(', ')} onChange={event => report('attributes', event.target.value.split(',').map(item => item.trim()).filter(Boolean))} />
                                </Field>
                                <Field label="Common units" note="Separate units with commas. Saved product units remain the source of truth.">
                                    <input placeholder="kg, L, m, pcs" value={(reportSettings.unit_examples || []).join(', ')} onChange={event => report('unit_examples', event.target.value.split(',').map(item => item.trim()).filter(Boolean))} />
                                </Field>
                            </div>
                            ...
                            <section className="report-layout-builder">
                                <div>
                                    <h3>Department columns</h3>
                                    <p>Select the departments to include. Their report columns and totals are generated automatically from the database.</p>
                                </div>
                                <div className="department-picker">
                                    {reportDepartments.map((department: any) => {
                                        const selected = !(reportSettings.department_ids || []).length || reportSettings.department_ids.includes(department.id);
                                        return (
                                            <button
                                                type="button" key={department.id} className={selected ? 'selected' : ''}
                                                onClick={() => {
                                                    const current = reportSettings.department_ids || [];
                                                    if (!current.length) report('department_ids', reportDepartments.filter((item: any) => item.id !== department.id).map((item: any) => item.id));
                                                    else toggleReportDepartment(department.id);
                                                }}
                                            >
                                                <span>{department.name}</span><small>{department.code}</small>
                                            </button>
                                        );
                                    })}
                                </div>
                                <small className="selection-note">{!(reportSettings.department_ids || []).length ? 'All active departments are included.' : `${reportSettings.department_ids.length} department(s) selected.`}</small>
                            </section>
                            <div className="report-design-save">
                                <div><b>Save this report design</b><span>The title, footer, tables and department columns will be used on the Reports page.</span></div>
                                <button type="submit" className="primary-btn" disabled={busy}><Save size={17} />{busy ? 'Saving…' : 'Save report design'}</button>
                            </div>
                        </Card>
                    )}
                </div>
