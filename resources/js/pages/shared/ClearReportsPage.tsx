    ...
    if (!days.length) {
        return <article className="panel noguchi-cutting-report" style={{marginBottom: 24}}>
            ...
            <div className="noguchi-cutting-table-wrap"><table><thead>
                <tr><th rowSpan={2}>DATE</th><th colSpan={4}>CUT PIECE DETAILS</th><th colSpan={3}>QUANTITIES</th></tr>
                <tr><th>FABRIC</th><th>STYLE</th><th>COLOR</th><th>SIZE</th><th>INPUT (ACCEPTED)</th><th>OUTPUT (SEWN)</th><th>WASTE</th></tr>
            </thead><tbody>
                <tr><td colSpan={8} className="empty-cell">No work was recorded for this period.</td></tr>
            </tbody></table></div>
            <footer><span>{data.factory?.name || 'NOGUCHI HOLDINGS LTD'} \u00b7 FACTORY OPERATIONS</span><span>Generated by ICYEREKEZO OMS</span></footer>
        </article>;
    }
    ...
    return <>{days.map((day: any) => <article key={day.date} className="panel noguchi-cutting-report" style={{marginBottom: 24}}>
        ...
        <div className="noguchi-cutting-table-wrap"><table><thead>
            ...
            {day.departments.map((department: any) => department.records.map((record: any, index: number) => {
                const fabricColor = String(record.product || '').match(/\b(green|blue|red|yellow|black|white|navy|grey|gray|brown|orange|purple|pink|beige|cream)\b/i)?.[1];
                return <tr key={`${department.department_id}-${index}`}>
                    <td>{new Date(`${day.date}T00:00:00`).toLocaleDateString()}</td>
                    <td><b>{record.product}</b></td>
                    <td>{'\u2014'}</td>
                    <td>{fabricColor || '\u2014'}</td>
                    <td>{'\u2014'}</td>
                    <td>{record.received > 0 ? <b>{number(record.received)}</b> : '\u2014'}</td>
                    <td>{record.completed > 0 ? <b style={{color: '#10b981'}}>{number(record.completed)}</b> : '\u2014'}</td>
                    <td>{record.damaged > 0 ? <span className="admin-status warning">{number(record.damaged)}</span> : '\u2014'}</td>
                </tr>;
            }))}
        </tbody></table></div>
        <footer><span>{data.factory?.name || 'NOGUCHI HOLDINGS LTD'} \u00b7 FACTORY OPERATIONS</span><span>Generated by ICYEREKEZO OMS</span></footer>
