        ...
        return <article className="panel noguchi-cutting-report" style={{marginBottom: 24}}>
            <header>
              ...
              <div className="noguchi-cutting-table-wrap" style={{ overflowX: 'auto' }}><table><thead>
                  <tr>
                      <th rowSpan={2} style={{verticalAlign: 'middle', borderRight: '2px solid #94a3b8'}}>DATE</th>
                      <th colSpan={3} style={{textAlign: 'center', padding: '8px', borderRight: '2px solid #94a3b8', backgroundColor: '#f1f5f9'}}>INPUT</th>
                      <th colSpan={4} style={{textAlign: 'center', padding: '8px', backgroundColor: '#f1f5f9'}}>OUTPUT</th>
                  </tr>
                  <tr>
                      <th>STYLE</th><th>SIZE</th><th style={{borderRight: '2px solid #94a3b8'}}>QTY</th>
              ...
              <tr>
                ...
                {day.departments.map((department: any) => department.records.map((record: any, index: number) => {
                  ...
                  return <tr key={`${department.department_id}-${index}`}>
                      <td style={{borderRight: '2px solid #94a3b8'}}>{new Date(`${day.date}T00:00:00`).toLocaleDateString()}</td>
                      <td>{style}</td><td>{info.size}</td><td style={{borderRight: '2px solid #94a3b8'}}>{record.received > 0 ? <b>{number(record.received)}</b> : '\u2014'}</td>
                      <td>{color}</td><td>{style}</td><td>{info.size}</td><td>{record.completed > 0 ? <b style={{color: '#10b981'}}>{number(record.completed)}</b> : '\u2014'}</td>
                  </tr>;
              }))}
          </tbody></table></div>
    </article>)}</>;
