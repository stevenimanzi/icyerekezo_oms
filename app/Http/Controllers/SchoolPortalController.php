...
class SchoolPortalController extends Controller
{
    private const PRICES = [
        ...
        'Secondary' => ['Sweater' => 8500, 'Uniform' => 12000, 'Sport' => 12000, 'Overall' => 15000, 'Overall Coat' => 9000, 'Polo Lacoste' => 12000, 'T-shirt' => 4000, 'Jumper' => 12000]
    ];
    ...
    public function storeOrder(Request $request): JsonResponse
    {
        ...
        $data=$request->validate(['academic_year'=>['required',Rule::in(['2025-2026','2026-2027','2027-2028','2028-2029','2029-2030','2030-2031'])],'lines'=>['required','array','min:1'],'lines.*.class_level'=>['required','string','max:30'],'lines.*.garment_category'=>['required',Rule::in(['Uniform','Sweater','Sport Uniform','T-shirt','Overall','Overall Coat'])],'lines.*.gender'=>['required',Rule::in(['Boy','Girl','Unisex'])],'lines.*.size'=>['required','string','max:20'],'lines.*.color'=>['nullable','string','max:255'],'lines.*.quantity_ordered'=>['required','integer','min:1']]);
        foreach($data['lines'] as $line){abort_if($line['garment_category']==='T-shirt'&&!preg_match('/^S[1-6]$/',$line['class_level']),422,'T-Shirts are available only for S1 to S6.');abort_if(in_array($line['garment_category'], ['Overall', 'Overall Coat'])&&!preg_match('/^S[4-6]$/',$line['class_level']),422,'Overalls and Overall Coats are available only for S4 to S6.');}
